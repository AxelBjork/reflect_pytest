...
import pytest
from reflect_pytest.udp_client import ComponentId, LogPayload, Severity, UdpClient
...
def udp(sil_process):
    ...
    with UdpClient() as client:
        client.register()
        # Give the bridge a moment to record our address before messages arrive.
        time.sleep(0.1)
        yield client
...
def test_inject_hello_world(udp):
    """Send our own Hello World; bridge injects it into the bus and echoes it back."""
    sent = LogPayload(
    ...
    # Drain messages until we see our own (the bridge may echo C++ messages first).
    ...
    pytest.fail("Did not receive injected 'Hello World from pytest' back over UDP")
