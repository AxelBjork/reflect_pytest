...
UdpBridge::UdpBridge(MessageBus& bus, uint16_t listen_port)
    : bus_(bus)
{
    udp_fd_ = ::socket(AF_INET, SOCK_DGRAM, 0);
    if (udp_fd_ < 0)
        throw std::runtime_error("UdpBridge: socket() failed");
    ...
    bus_.subscribe_all([this](RawMessage msg) {
        forward_to_udp(std::move(msg));
    });
...
void UdpBridge::rx_loop()
{
    ...
    while (true) {
        ...
        if (FD_ISSET(udp_fd_, &fds)) {
            ...
            bus_.publish_raw(static_cast<MsgId>(id_raw),
                             buf + sizeof(uint16_t),
