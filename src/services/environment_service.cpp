...
void EnvironmentService::on_message(const ipc::EnvironmentPayload& env) {
  ...
  if (it != cache_.end()) {
    **it = env;
    ...
    cache_.push_back(std::make_shared<ipc::EnvironmentPayload>(env));
  }
...
void EnvironmentService::on_message(const ipc::ResetRequestPayload& req) {
  std::lock_guard lk{mu_};
  cache_.clear();
  logger_.info("Environment cache cleared on reset.");
}

void EnvironmentService::on_message(const ipc::InternalEnvRequestPayload& req) {
  std::lock_guard lk{mu_};
  for (const auto& env : cache_) {
    if (req.x >= env->bounds.min_pt.x && req.x <= env->bounds.max_pt.x &&
      ...
      bus_.publish<ipc::MsgId::InternalEnvData>(ipc::InternalEnvDataPayload{env});
      return;
    }
  }
  ...
  if (now - last_request_time_ > std::chrono::milliseconds(500)) {
    ...
    ipc::EnvironmentRequestPayload ext_req;
    ext_req.target_location = {req.x, req.y};
    bus_.publish<ipc::MsgId::EnvironmentRequest>(ext_req);
